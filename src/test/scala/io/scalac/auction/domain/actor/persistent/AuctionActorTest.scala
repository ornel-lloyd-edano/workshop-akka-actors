...
class AuctionActorTest extends ScalaTestWithActorTestKit(s"""
  ...
  "AuctionActor in the closed state" should {
    "accept AddLot message while in this state and reply with LotCreated" in {
      ...
      probe.expectMessage(AuctionActor.LotCreated(auctionId, "1", Some("notebooks from Nikola Tesla's library"), Some(BigDecimal(1000))))
      probe.expectMessage(AuctionActor.LotCreated(auctionId, "2", Some("old stuff from attic"), Some(BigDecimal(1000))))
      probe.expectMessage(AuctionActor.LotCreated(auctionId, "3", Some("large fossilized eggs"), Some(BigDecimal(1000))))
    }
    ...
    "accept RemoveLot message while in this state and reply with LotRemoved" in {
      ...
      probe.expectMessage(AuctionActor.LotCreated(auctionId, "4", Some("notebooks from Nikola Tesla's library"), Some(BigDecimal(1000))))
      probe.expectMessage(AuctionActor.LotCreated(auctionId, "5", Some("old stuff from attic"), Some(BigDecimal(1000))))
      probe.expectMessage(AuctionActor.LotCreated(auctionId, "6", Some("large fossilized eggs"), Some(BigDecimal(1000))))
      probe.expectMessage(AuctionActor.LotCreated(auctionId, "7", Some("pandora's box"), Some(BigDecimal(9999))))
      probe.expectMessage(AuctionActor.LotCreated(auctionId, "8", Some("old pirate chest"), Some(BigDecimal(7777))))
